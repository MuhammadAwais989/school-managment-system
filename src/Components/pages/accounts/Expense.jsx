  ...
  const fetchChartData = async () => {
    try {
      ...
      const [incomeRes, expenseRes] = await Promise.all([
        ...
        axios.get(
          `${BaseURL}/accounts/expense/yearly-summary?year=${chartYear}`
        ),
      ]);
      ...
      <div className="lg:pl-[90px] max-sm:mt-[-79px] max-sm:pt-[79px] sm:pt-2 pr-2 pb-2 max-sm:pt-1 max-sm:pl-2 max-lg:pl-[90px] bg-gray-50 w-full min-h-screen">
        <div className="bg-white w-full h-full shadow-md rounded-md px-4 max-sm:px-4 pt-2 pb-3">
          ...
          <PageTitle
            ...
            description="Track and manage all expenses and expenditures"
            icon={LuTrendingDown}
          ...
          <div className="flex justify-between items-center flex-wrap gap-4 mb-6 pt-5">
            {/* Filter */}
            <div className="relative">
              <label className="absolute -top-3 left-2 bg-white px-1 text-sm text-gray-500">
                Filter By
              </label>
              <select
                value={filters}
            ...
            <table className="min-w-full table-auto ">
              <thead>
                <tr className="bg-gradient-to-r from-[#F2F5FF] to-[#E7F0FF]">
                  <th className="px-6 py-4 text-left text-sm font-medium text-[#3E54AC]">
                    Date
                  </th>
                  <th className="px-6 py-4 text-left text-sm font-medium text-[#3E54AC]">
                    Category
                  </th>
                  <th className="px-6 py-4 text-right text-sm font-medium text-[#3E54AC]">
                    Amount
                  </th>
                  <th className="px-6 py-4 text-left text-sm font-medium text-[#3E54AC]">
                    Status
              ...
              <tbody className="divide-y divide-[#E0E7FF] bg-white">
                {expenses.map((item) => {
                  ...
                  return (
                    <tr
                      key={item._id}
                      className="hover:bg-[#F2F5FF] transition-colors"
                    >
                      <td className="whitespace-nowrap px-6 py-4 text-sm font-medium text-[#1A237E]">
                        {displayDate.toLocaleDateString("en-PK", {
                          month: "short",
                          day: "numeric",
                        })}
                      </td>
              ...
              <tfoot className="bg-[#F2F5FF]">
                <tr>
                  <td
                    colSpan="4"
                    className="px-6 py-3 text-right text-sm text-[#3E54AC] font-medium"
                  >
                    Total: Rs{" "}
                    {expenses.reduce(
